...
function getDeckPlay(greenDeckPlay, brownDeckPlay, blueDeckPlay) {
    ...
    const cardsAmount = document.querySelectorAll('.counter');
    const stage1 = [];
    const stage2 = [];
    const stage3 = [];  
...
function setTrackableAncients() {
    const ancients = document.querySelectorAll('.ancients_image');
...
function setActiveAncient(event) {
    ...
    for (let i = 0; i < ancients.length; i++) {
        ancients[i].classList.remove('ancients_card-active');
    }

    ancient[0].classList.add('ancients_card-active');
